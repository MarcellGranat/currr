...
cp_map_chr <- function(.x, .f, ..., name = NULL, cp_options = list()) {
  ...
  get_fun <- function(x) {
    ...
    as.character(ifelse(is.null(x), as.character(NA), x))
  }

  get_fun <- purrr::safely(get_fun, as.numeric(NA), quiet = TRUE)

  cp_map(.x = .x, .f = .f, ... = ..., name = name, cp_options = cp_options) |>
    purrr::map_chr(~ get_fun(.)[[1]])
}
